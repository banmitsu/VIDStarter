...
def dirwalk(path, fmt='.xml'):
    a = []
    for s in os.listdir(path):
        if os.path.isfile(os.path.join(path, s)) & s.endswith(fmt):
            a.append(os.path.join(path, s))
        elif os.path.isdir(os.path.join(path, s)):
            a = a + dirwalk(os.path.join(path, s))
    return a

def get_ordered_name_XML(path_bbox_dataset):
    assert os.path.isdir(path_bbox_dataset) == True
    a = dirwalk(path_bbox_dataset, fmt='.xml')
    return sorted(a)

def get_ordered_name_JPG(path_img_dataset):
    assert os.path.isdir(path_img_dataset) == True
    a = dirwalk(path_img_dataset, fmt='.jpg')
    return sorted(a)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bb_folder', default='/volHDD/home/tracking_dataset/ilsvrc_vid/ILSVRC2015/Annotations', type=str)
